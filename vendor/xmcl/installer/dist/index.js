...
var require_streamx = __commonJS({
  "../../node_modules/.pnpm/streamx@2.16.1/node_modules/streamx/index.js"(exports2, module2) {
...
var require_b4a = __commonJS({
  "../../node_modules/.pnpm/b4a@1.6.6/node_modules/b4a/index.js"(exports2, module2) {
...
__export(index_exports, {
  ...
  DEFAULT_META_URL_FABRIC: () => DEFAULT_META_URL_FABRIC,
  DEFAULT_META_URL_QUILT: () => DEFAULT_META_URL_QUILT,
  ...
  PostProcessValidationFailedError: () => PostProcessValidationFailedError,
  classpathEntryToLibraryName: () => classpathEntryToLibraryName,
  completeInstallation: () => completeInstallation,
  completeInstallationByError: () => completeInstallationByError,
  detectLibc: () => detectLibc,
  diagnoseFile: () => diagnoseFile,
  diagnoseLibraries: () => diagnoseLibraries,
  diagnoseProcessorOutputs: () => diagnoseProcessorOutputs,
  ...
  getFabricGames: () => getFabricGames,
  getFabricLoaderArtifact: () => getFabricLoaderArtifact,
  getFabricLoaders: () => getFabricLoaders,
  getForgeVersionList: () => getForgeVersionList,
  getLabyModAddon: () => getLabyModAddon,
  getLabyModAddonIndex: () => getLabyModAddonIndex,
  ...
  getQuiltGames: () => getQuiltGames,
  getQuiltLoaderVersionsByMinecraft: () => getQuiltLoaderVersionsByMinecraft,
  getQuiltLoaders: () => getQuiltLoaders,
  getVersionJsonFromLoaderArtifact: () => getVersionJsonFromLoaderArtifact,
  getVersionList: () => getVersionList,
  installAssets: () => installAssets,
  installByProfile: () => installByProfile,
  installFabric: () => installFabric,
  installFabricByLoaderArtifact: () => installFabricByLoaderArtifact,
  installForge: () => installForge,
  installJavaRuntime: () => installJavaRuntime,
  installJavaRuntimeWithJson: () => installJavaRuntimeWithJson,
  installLabyMod4: () => installLabyMod4,
  installLabyModAddon: () => installLabyModAddon,
  installLabyModFabricAddon: () => installLabyModFabricAddon,
  installLabyModForgeAddon: () => installLabyModForgeAddon,
  installLibraries: () => installLibraries,
  installMinecraft: () => installMinecraft,
  installMinecraftJar: () => installMinecraftJar,
  installNeoForge: () => installNeoForge,
  installOptifine: () => installOptifine,
  installQuiltVersion: () => installQuiltVersion,
  installResolvedAssets: () => installResolvedAssets,
  installResolvedLibraries: () => installResolvedLibraries,
  installZuluJava: () => installZuluJava,
  isForgeInstallerEntries: () => isForgeInstallerEntries,
  isInstallError: () => isInstallError,
  isLabyModAddonCompatible: () => isLabyModAddonCompatible,
  isLegacyForgeInstallerEntries: () => isLegacyForgeInstallerEntries,
  mergeInstallIssue: () => mergeInstallIssue,
  onDownloadMultiple: () => onDownloadMultiple,
  onDownloadSingle: () => onDownloadSingle,
  onProgress: () => onProgress,
  onState: () => onState,
  parseArgumentsFromArgsFile: () => parseArgumentsFromArgsFile,
  parseJavaVersion: () => parseJavaVersion,
  parseJavaVersionOutput: () => parseJavaVersionOutput,
  resolveJava: () => resolveJava,
  resolveJavaWithDiagnostic: () => resolveJavaWithDiagnostic,
  resolveLibraryDownloadUrls: () => resolveLibraryDownloadUrls,
  resolveProcessors: () => resolveProcessors,
  scanLocalJava: () => scanLocalJava,
  selectZuluJRE: () => selectZuluJRE,
...
var import_utils2 = require("@xmcl/core/utils");
...
function missing(target) {
  return (0, import_promises.access)(target).then(
    () => false,
    () => true
  );
}
...
async function diagnoseFile({
  ...
  const checksumFunc = (options == null ? void 0 : options.checksum) ?? import_core.checksum;
  const signal = options == null ? void 0 : options.signal;
  const fileExisted = await (0, import_promises2.access)(file).then(
    () => true,
    () => false
  );
  ...
  if (!fileExisted) {
    ...
    const fstat = await (0, import_promises2.stat)(file).catch(() => ({ size: 0 }));
    if (fstat.size === 0) {
      issue = true;
    }
  }
...
function onDownloadMultiple(tracker, phase, payload) {
  const parent = new import_file_transfer.ProgressTrackerMultiple();
  tracker == null ? void 0 : tracker({ phase, payload: { ...payload, progress: parent } });
  return parent;
}
function onDownloadSingle(tracker, phase, payload) {
  const single = new import_file_transfer.ProgressTrackerSingle();
  tracker == null ? void 0 : tracker({ phase, payload: { ...payload, progress: single } });
  return single;
}
...
async function installAssets(version, options = {}) {
  var _a, _b;
  const folder = import_core2.MinecraftFolder.from(version.minecraftDirectory);
  if ((_b = (_a = version.logging) == null ? void 0 : _a.client) == null ? void 0 : _b.file) {
    ...
    await diagnoseFile(
      ...
      await (0, import_file_transfer2.download)({
        url: [file.url],
        destination: folder.getLogConfig(file.id),
        expectedTotal: file.size,
        ...(0, import_file_transfer2.getDownloadBaseOptions)(options),
  ...
  const fetchAssetIndex = async () => {
    ...
    for (const url of urls) {
      try {
        ...
        await (0, import_promises3.writeFile)(jsonPath, JSON.stringify(json));
        return json;
      } catch {
      }
    }
  ...
  const readJson = async () => (0, import_promises3.readFile)(jsonPath, "utf-8").then((b) => JSON.parse(b));
  ...
  const assetIndex = await readJson().catch(async (e) => {
    ...
    await (0, import_util.promisify)(import_fs.link)(
...
async function diagnoseAssets(assetObjects, minecraft, options) {
  ...
  const issues = await Promise.all(
    assetObjects.map(async (asset) => {
      ...
      if (options == null ? void 0 : options.strict) {
        ...
        const { size: realSize } = await (0, import_promises3.stat)(assetPath).catch(() => ({ size: -1 }));
        if (signal == null ? void 0 : signal.aborted) return;
        if (realSize !== size) {
          const issue = await diagnoseFile(
            {
  ...
  return issues.filter(import_utils2.isNotNull);
}
...
async function installFabricByLoaderArtifact(loader, minecraft, options = {}) {
  const folder = import_core3.MinecraftFolder.from(minecraft);
  ...
  await (0, import_promises4.writeFile)(jsonFile, JSON.stringify(version, null, 4));
  return version.id;
}
async function installFabric(options) {
  ...
  const minecraft = import_core3.MinecraftFolder.from(options.minecraft);
  ...
  await (0, import_promises4.writeFile)(jsonPath, JSON.stringify(content));
  return content.id;
}

// forge.ts
var import_core6 = require("@xmcl/core");
var import_file_transfer4 = require("@xmcl/file-transfer");
...
async function getForgeVersionList(options = {}) {
  ...
  const response = await doFetch(options, url);
  const body = (0, import_forge_site_parser.parse)(await response.text());
  return body;
}

// libraries.ts
var import_core4 = require("@xmcl/core");
var import_utils9 = require("@xmcl/core/utils");
...
async function installResolvedLibraries(libraries, minecraft, option = {}) {
  const folder = import_core4.MinecraftFolder.from(typeof minecraft === "string" ? minecraft : minecraft.root);
  await diagnoseLibraries(libraries, folder, {
    ...
    const results = await (0, import_file_transfer3.downloadMultiple)({
      ...
      ...(0, import_file_transfer3.getDownloadBaseOptions)(option)
    });
...
async function diagnoseLibraries(libraries, minecraft, options) {
    ...
    libraries.map(async (lib) => {
      ...
      if (!(options == null ? void 0 : options.strict)) {
        if (lib.download.sha1) {
          ...
          try {
            const zip = await (0, import_unzip.open)(libPath);
            try {
              for await (const _ of (0, import_unzip.walkEntriesGenerator)(zip)) {
              }
            } finally {
              zip.close();
            }
        ...
        const { size: realSize } = await (0, import_promises5.stat)(libPath).catch(() => ({ size: -1 }));
        if (signal == null ? void 0 : signal.aborted) return;
  ...
  return issues.filter(import_utils9.isNotNull);
}
...
async function diagnoseProfile(installProfile, minecraftLocation, side = "client") {
  const mc = import_core5.MinecraftFolder.from(minecraftLocation);
  const processors = resolveProcessors(side, installProfile, mc);
  const issues = await Promise.all(
    import_core5.Version.resolveLibraries(installProfile.libraries).map(async (lib) => {
...
async function installByProfile(installProfile, minecraft, options = {}) {
  ...
  if (side === "client") {
    const versionJson = await (0, import_promises6.readFile)(
    ...
    let jar;
    if (!txtPath) {
      const info = import_core5.LibraryInfo.resolve(installProfile.path);
      const libPath = minecraftFolder.getLibraryByPath(info.path);
      jar = libPath;
    } else {
      const content = await (0, import_promises6.readFile)(txtPath, "utf-8");
      jar = parseArgumentsFromArgsFile(content, (0, import_path4.dirname)(txtPath), serverProfile);
    }
    if (jar) {
    ...
    if (neoFormVersion) {
      ...
      for (const name of candidates) {
        const libPath = minecraftFolder.getLibraryByPath(import_core5.LibraryInfo.resolve(name).path);
    ...
    if (forgeShim) {
      let zip;
      try {
        zip = await (0, import_unzip2.open)(minecraftFolder.getLibraryByPath(import_core5.LibraryInfo.resolve(forgeShim.name).path));
        for await (const entry of (0, import_unzip2.walkEntriesGenerator)(zip)) {
          if (entry.fileName === "bootstrap-shim.list") {
            const content = await (0, import_unzip2.readEntry)(zip, entry).then(
    ...
    const clientVersionJson = await (0, import_promises6.readFile)(
      minecraftFolder.getVersionJson(installProfile.version)
    ...
    if (typeof cpValue === "string") {
      ...
      for (const entry of cpValue.split(import_path4.delimiter)) {
        if (!entry) continue;
    ...
    await (0, import_promises6.writeFile)(
      (0, import_path4.join)(minecraftFolder.getVersionRoot(serverProfile.id), "server.json"),
      JSON.stringify(serverProfile, null, 4)
    );
    const resolvedLibraries = import_core5.Version.resolveLibraries(serverProfile.libraries);
...
function parseArgumentsFromArgsFile(content, parentDir, serverProfile) {
  ...
  for (; i < args.length; i++) {
    const arg = args[i];
    if (arg === "-jar") {
      jar = (0, import_path4.join)(parentDir, args[i + 1] ?? "");
      i += 2;
      break;
    }
    if (!arg.startsWith("-")) {
...
async function parseJar(minecraftFolder, jar, installProfile, serverVersion) {
  ...
  try {
    const jsonContent = JSON.parse(
      await (0, import_promises6.readFile)(minecraftFolder.getVersionJson(installProfile.version), "utf-8")
    );
    zip = await (0, import_unzip2.open)(jar, { lazyEntries: true, autoClose: false });
    const [entry] = await (0, import_unzip2.filterEntries)(zip, ["META-INF/MANIFEST.MF"]);
    if (entry) {
      const manifestContent = await (0, import_unzip2.readEntry)(zip, entry).then((b) => b.toString());
      const result = parseManifest(manifestContent);
      serverVersion.mainClass = result.mainClass;
      const cp = [
        ...result.classPath,
        (0, import_path4.relative)(minecraftFolder.libraries, jar).replaceAll(import_path4.sep, "/")
      ];
...
async function isEmptyOrCorruptArchive(file, signal) {
  let zip;
  try {
    zip = await (0, import_unzip2.open)(file, { lazyEntries: true, autoClose: false });
...
async function postProcessOne(mc, proc, options) {
  ...
  try {
    await new Promise((resolve, reject) => {
      const process2 = ((options == null ? void 0 : options.spawn) ?? import_child_process2.spawn)(options.java ?? "java", cmd, {
...
function extractEntryTo(zip, e, dest) {
  return (0, import_unzip3.openEntryReadStream)(zip, e).then((stream) => (0, import_promises8.pipeline)(stream, (0, import_fs2.createWriteStream)(dest)));
}
async function installLegacyForgeFromUniversalZip(forgeZip, mc, forgeVersion, mcVersion) {
  const minecraftZip = await (0, import_unzip3.open)(mc.getVersionJar(mcVersion), {
    lazyEntries: true,
    autoClose: false
  });
  const forgeEntries = await (0, import_unzip3.readAllEntries)(forgeZip);
  const mcEntries = await (0, import_unzip3.readAllEntries)(minecraftZip);
  ...
  for (const [k, [e, zip]] of Object.entries(finalZipEntries)) {
    finalZip.addReadStream(await (0, import_unzip3.openEntryReadStream)(zip, e), e.fileName);
  }
  finalZip.end();
  const dest = mc.getLibraryByPath(
    `net/minecraftforge/forge/${forgeVersion}/forge-${forgeVersion}.jar`
  );
  await ensureDir((0, import_path5.dirname)(dest));
  await (0, import_promises8.pipeline)(finalZip.outputStream, (0, import_fs2.createWriteStream)(dest));
  ...
  await (0, import_promises7.writeFile)(mc.getVersionJson(versionId), JSON.stringify(versionJson, null, 4));
  return versionId;
}
async function installLegacyForgeFromZip(zip, entries, profile, mc, jarFilePath, options) {
  ...
  await Promise.all([
    (0, import_promises7.writeFile)(versionJsonPath, JSON.stringify(versionJson, void 0, 4)),
...
async function unpackForgeInstaller(zip, entries, profile, mc, jarPath, options) {
  ...
  const mavenLibVersionPath = (0, import_path5.dirname)(jarPath);
  const unpackData = (entry) => {
    promises.push(
      extractEntryTo(
        zip,
        entry,
        (0, import_path5.join)(mavenLibVersionPath, entry.fileName.substring("data/".length))
      )
    );
  };
  await ensureFile(versionJsonPath);
  ...
  const mavenPaths = (0, import_path5.relative)(mc.libraries, mavenLibVersionPath).split(import_path5.sep);
  ...
  const extractLzma = async (entry, classifier) => {
    ...
    const binPath = mc.getLibraryByPath(import_core6.LibraryInfo.resolve(maven).path);
  ...
  promises.push(
    (0, import_promises7.writeFile)(installJsonPath, JSON.stringify(profile)),
    (0, import_promises7.writeFile)(versionJsonPath, JSON.stringify(versionJson))
  );
...
async function walkForgeInstallerEntries(zip, forgeVersion) {
  const [
    ...
    userJvmArgs,
    winArgs
  ] = await (0, import_unzip3.filterEntries)(zip, [
...
async function downloadForgeJar(forgeVersion, mcVersion, installer, minecraft, options, legacy) {
  ...
  if (library.download.sha1) {
    await diagnoseFile(
      ...
      await (0, import_file_transfer4.download)({
        url: urls,
        destination: installJarPath,
        ...(0, import_file_transfer4.getDownloadBaseOptions)(options),
...
async function installForge(version, minecraft, options = {}) {
  ...
  if (isLegacy) {
    const forgeZip = await (0, import_unzip3.open)(jarPath, { lazyEntries: true, autoClose: false });
...
// installer.ts
var import_core8 = require("@xmcl/core");
var import_promises10 = require("fs/promises");
var import_path7 = require("path");

// minecraft.ts
var import_core7 = require("@xmcl/core");
var import_file_transfer5 = require("@xmcl/file-transfer");
...
async function installMinecraftJar(version, options = {}) {
  const folder = import_core7.MinecraftFolder.from(version.minecraftDirectory);
  const side = options.side ?? "client";
  if (version.downloads[side]) {
    ...
    await diagnoseFile(
      ...
      return (0, import_file_transfer5.download)({
        url: jarUrls,
        destination: jarDestination,
        ...(0, import_file_transfer5.getDownloadBaseOptions)(options),
        ...
        if (post) {
          await (0, import_promises9.unlink)(jarDestination).catch(() => {
          });
...
async function installMinecraft(versionMeta, minecraft, options = {}) {
  const folder = import_core7.MinecraftFolder.from(minecraft);
  const version = await import_core7.Version.parse(folder, versionMeta.id).catch(async (e) => {
    if (options.diagnose) {
      throw e;
    }
    if (!(0, import_core7.isBadVersionJsonError)(e) && !(0, import_core7.isCorruptedVersionJsonError)(e) && !(0, import_core7.isMissingVersionJsonError)(e)) {
      throw e;
    }
    ...
    await (0, import_file_transfer5.download)({
      url: jsonUrls,
    ...
    return import_core7.Version.parse(folder, versionMeta.id);
  });
  const side = options.side ?? "client";
  ...
  if (side === "server") {
    ...
    const server = {
      ...
      jar: (0, import_path6.relative)(folder.libraries, jarPath).replaceAll(import_path6.sep, "/"),
      arguments: {
        game: [],
        jvm: []
      },
    ...
    await (0, import_promises9.writeFile)(
      (0, import_path6.join)(folder.getVersionRoot(versionMeta.id), "server.json"),
...
async function readProfile(versionDir) {
  const installProfilePath = (0, import_path7.join)(versionDir, "install_profile.json");
  try {
    const installProfile = JSON.parse(await (0, import_promises10.readFile)(installProfilePath, "utf8"));
...
async function completeInstallation(version, options = {}) {
  ...
  const folder = import_core8.MinecraftFolder.from(version.minecraftDirectory);
  ...
  if (options.diagnose && Object.keys(issue).length > 0) {
    if (issue.libraries && issue.libraries.length > 0) {
      ...
      if (forges.length > 0) {
        const header = (0, import_core8.getResolvedVersionHeader)(version);
...
async function resolveJavaWithDiagnostic(path) {
  ...
  return new Promise((resolve) => {
    (0, import_child_process3.exec)(`"${path}" -version`, (error, stdout, stderr) => {
...
async function getPotentialJavaLocations() {
  ...
  if (process.env.JAVA_HOME) {
    unchecked.add((0, import_path8.join)(process.env.JAVA_HOME, "bin", javaFile));
  }
  const which = () => new Promise((resolve) => {
    (0, import_child_process3.exec)("which java", (_error, stdout) => {
      if (!_error) resolve(stdout.replace("\n", ""));
      else resolve("");
    }).once("error", () => resolve(""));
  });
  const where = () => new Promise((resolve) => {
    (0, import_child_process3.exec)("where java", (_error, stdout) => {
...
async function dedupJreExecutables(files) {
  ...
  for (const file of files) {
    const fstat = await (0, import_promises11.stat)(file).catch(() => ({ ino: -1 }));
...
// java-runtime.ts
var import_file_transfer6 = require("@xmcl/file-transfer");
var import_promises12 = require("fs/promises");
var import_path9 = require("path");

// java-runtime.browser.ts
var import_core9 = require("@xmcl/core");
...
async function fetchJavaRuntimeManifest(options = {}) {
  ...
  const platform2 = options.platform ?? (0, import_core9.getPlatform)();
...
async function downloadFiles(destination, options, manifest) {
  ...
  const diagnoseResults = await Promise.all(
    fileEntries.map(async ([file, entry]) => {
      ...
      const rawDest = (0, import_path9.join)(destination, file);
...
${errors.join("\n")}`);
  ...
  if (filesToDownload.length > 0) {
    await (0, import_file_transfer6.downloadMultiple)({
      options: filesToDownload,
      tracker: onDownloadMultiple(options.tracker, "java-runtime.file", { path: destination }),
      ...(0, import_file_transfer6.getDownloadBaseOptions)(options)
    });
  }
  if (unpackLzma) {
    await Promise.all(
  ...
  await Promise.all(
    Object.entries(manifest.files).filter(([file, entry]) => entry.type !== "file").map(async ([file, entry]) => {
      const dest = (0, import_path9.join)(destination, file);
      if (entry.type === "directory") {
        await ensureDir(dest);
      } else if (entry.type === "link") {
        await (0, import_promises12.link)(dest, (0, import_path9.join)((0, import_path9.dirname)(dest), entry.target)).catch(() => {
        });
      }
    })
  );
...
async function installJavaRuntimeWithJson(options) {
  ...
  const jsonPath = (0, import_path9.join)(destination, "manifest.json");
  const readManifest = async () => {
    const content = await (0, import_promises12.readFile)(jsonPath);
  ...
  if (manifestIssue) {
    ...
    const downloadOptions = (0, import_file_transfer6.getDownloadBaseOptions)(options);
    const manifestUrl = normalizeUrls2(target.manifest.url, options.apiHost);
    await (0, import_file_transfer6.download)({
      destination: jsonPath,
      url: manifestUrl,
      ...downloadOptions,
...
// labymod.ts
var import_core10 = require("@xmcl/core");
var import_file_transfer7 = require("@xmcl/file-transfer");
...
async function createLabyModJson(manifest, tag, folder, environment, options) {
  ...
  versionJson.libraries.push(
    ...libraries.map((l) => ({
      ...
      downloads: {
        artifact: {
          path: import_core10.LibraryInfo.resolve(l.name).path,
          sha1: l.sha1,
          size: l.size,
          url: l.url
        }
  ...
  await ensureDir((0, import_path10.dirname)(versionPath));
  await (0, import_promises13.writeFile)(versionPath, JSON.stringify(versionJson, null, 4));
  return versionJson.id;
}
async function installLabyMod4(manifest, tag, minecraft, options = {}) {
  const folder = import_core10.MinecraftFolder.from(minecraft);
  ...
  if (assetsToDownload.length > 0) {
    await (0, import_file_transfer7.downloadMultiple)({
      options: assetsToDownload.map((r) => ({
        url: r.url,
        destination: r.destination
      })),
      ...(0, import_file_transfer7.getDownloadBaseOptions)(options),
...
async function installLabyModAddonImpl(addon, minecraft, options) {
  ...
  const destination = (0, import_path10.join)(folder.getPath("labymod-neo", "addons"), `${addon.namespace}.jar`);
  const issue = await diagnoseFile(
    {
      file: destination,
  ...
  if (issue) {
    await (0, import_file_transfer7.download)({
      url,
      destination,
      ...(0, import_file_transfer7.getDownloadBaseOptions)(options),
...
// neoforge.ts
var import_core11 = require("@xmcl/core");
var import_file_transfer8 = require("@xmcl/file-transfer");
var import_unzip4 = require("@xmcl/unzip");
...
async function downloadNeoForgedInstaller(project, version, minecraft, options) {
  ...
  const library = import_core11.Version.resolveLibrary({
  ...
  const urls = resolveLibraryDownloadUrls(library, { ...options, mavenHost });
  const installJarPath = minecraft.getLibraryByPath(library.path);
  const doDownload = () => (0, import_file_transfer8.download)({
    url: urls,
    destination: installJarPath,
    ...(0, import_file_transfer8.getDownloadBaseOptions)(options),
...
async function installNeoForge(project, version, minecraft, options = {}) {
  ...
  const mc = import_core11.MinecraftFolder.from(minecraft);
  const jarPath = await downloadNeoForgedInstaller(project, version, mc, options);
  const zip = await (0, import_unzip4.open)(jarPath, { lazyEntries: true, autoClose: false });
  const entries = await walkForgeInstallerEntries(zip, forgeVersion);
  if (!entries.installProfileJson) {
    throw new BadForgeInstallerJarError(jarPath, "install_profile.json");
  }
  const profile = await (0, import_unzip4.readEntry)(zip, entries.installProfileJson).then((b) => b.toString()).then(JSON.parse);
  if (isForgeInstallerEntries(entries)) {
    const versionId = await unpackForgeInstaller(zip, entries, profile, mc, jarPath, options);
    await installByProfile(profile, minecraft, options);
    return versionId;
...
var import_core12 = require("@xmcl/core");
var import_unzip5 = require("@xmcl/unzip");
...
async function installOptifine(installer, minecraft, options = {}) {
  ...
  const mc = import_core12.MinecraftFolder.from(minecraft);
  const zip = await (0, import_unzip5.open)(installer);
  const entries = await (0, import_unzip5.readAllEntries)(zip);
  const record = (0, import_unzip5.getEntriesRecord)(entries);
  ...
  const launchWrapperVersion = launchWrapperVersionEntry ? await (0, import_unzip5.readEntry)(zip, launchWrapperVersionEntry).then((b) => b.toString()) : void 0;
  const buf = await (0, import_unzip5.readEntry)(zip, entry);
  const reader = new import_asm.ClassReader(buf);
  class OptifineVisitor extends import_asm.ClassVisitor {
    fields = {};
  ...
  const visitor = new OptifineVisitor(import_asm.Opcodes.ASM5);
  ...
  await ensureFile(versionJSONPath);
  await (0, import_promises15.writeFile)(versionJSONPath, JSON.stringify(versionJSON, null, 4), {
    signal: options.signal
  });
  progress.progress = 1;
  ...
  if (launchWrapperEntry) {
    ...
    await (0, import_promises15.writeFile)(wrapperDest, await (0, import_unzip5.readEntry)(zip, launchWrapperEntry), {
      signal: options.signal
    });
    progress.progress = 2;
  }
...
// quilt.ts
var import_core13 = require("@xmcl/core");
...
async function installQuiltVersion(options) {
  ...
  const response = await doFetch(options, url);
  const content = await response.json();
  const minecraft = import_core13.MinecraftFolder.from(options.minecraft);
  if (options.inheritsFrom) {
    content.inheritsFrom = options.inheritsFrom;
  ...
  const jsonPath = side === "client" ? minecraft.getVersionJson(content.id) : minecraft.getVersionServerJson(content.id);
  await ensureFile(jsonPath);
  await (0, import_promises16.writeFile)(jsonPath, JSON.stringify(content));
  return content.id;
}

// zulu.ts
var import_file_transfer9 = require("@xmcl/file-transfer");
var import_unzip6 = require("@xmcl/unzip");
...
async function installZuluJava(jre, options) {
  ...
  const packedFile = (0, import_path11.join)(destination, (0, import_path11.basename)(jre.url));
  ...
  await (0, import_file_transfer9.download)({
    url: jre.url,
    destination: packedFile,
    expectedTotal: jre.size,
    tracker: downloadTracker,
    ...(0, import_file_transfer9.getDownloadBaseOptions)(options),
    signal: options.abortSignal
  });
  try {
    ...
    try {
      await (0, import_promises17.stat)(packedFile).then(() => (0, import_promises17.unlink)(packedFile));
    } catch {
    }
  }
}
async function extractTarGz(packedFile, destination, jre, tracker, abortSignal) {
  ...
  const allPipe = [
    (0, import_promises18.pipeline)((0, import_fs3.createReadStream)(packedFile), (0, import_zlib.createGunzip)(), extractStream)
  ];
  let first = "";
  let substring = 0;
  const links = [];
  ...
  try {
    for await (const entry of extractStream) {
      ...
      const filePath = (0, import_path11.join)(destination, entry.header.name.substring(substring));
      const size = entry.header.size ?? 0;
      if (entry.header.type === "directory") {
        await ensureDir(filePath);
      } else if (entry.header.linkname && entry.header.type === "symlink") {
        links.push({
          path: (0, import_path11.join)(destination, entry.header.linkname),
          linkTo: filePath
        });
      } else if (entry.header.type === "file") {
        progress.total += size;
        await ensureDir((0, import_path11.dirname)(filePath));
        const writeStream = (0, import_fs3.createWriteStream)(filePath);
        ...
        allPipe.push((0, import_promises18.pipeline)(entry, writeStream));
      }
    }
    for (const link3 of links) {
      ...
      try {
        await (0, import_promises17.symlink)(link3.path, link3.linkTo);
      } catch {
      }
    }
...
async function extractZip(packedFile, destination, jre, tracker, abortSignal) {
  ...
  const zipFile = await (0, import_unzip6.open)(packedFile);
  const abortHandler = () => {
    zipFile.close();
  };
  abortSignal == null ? void 0 : abortSignal.addEventListener("abort", abortHandler);
  try {
    ...
    const prefix = (0, import_path11.basename)(jre.url).slice(0, -4) + "/";
    const entries = await (0, import_unzip6.readAllEntries)(zipFile).then(
    ...
    for (const entry of entries) {
      ...
      const file = (0, import_path11.join)(destination, relativePath);
      trackerProgress.total += entry.uncompressedSize;
      const readStream = await (0, import_unzip6.openEntryReadStream)(zipFile, entry);
      await ensureFile(file);
      const writeStream = (0, import_fs3.createWriteStream)(file);
      const originalWrite = writeStream.write.bind(writeStream);
      writeStream.write = function(chunk, ...args) {
      ...
      promises.push((0, import_promises18.pipeline)(readStream, writeStream));
    }
    await Promise.all(promises);
  } finally {
    abortSignal == null ? void 0 : abortSignal.removeEventListener("abort", abortHandler);

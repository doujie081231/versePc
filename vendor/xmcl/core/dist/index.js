...
__export(index_exports, {
  DEFAULT_EXTRA_JVM_ARGS: () => DEFAULT_EXTRA_JVM_ARGS,
  ...
  Version: () => Version,
  checksum: () => checksum,
  createMinecraftProcessWatcher: () => createMinecraftProcessWatcher,
  createQuickPlayMultiplayer: () => createQuickPlayMultiplayer,
  findLabyModVersion: () => findLabyModVersion,
  findNeoforgeVersion: () => findNeoforgeVersion,
  generateArguments: () => generateArguments,
  generateArgumentsServer: () => generateArgumentsServer,
  getPlatform: () => getPlatform,
  getResolvedVersionHeader: () => getResolvedVersionHeader,
  isBadVersionJsonError: () => isBadVersionJsonError,
  isCorruptedVersionJsonError: () => isCorruptedVersionJsonError,
  isFabricLoaderLibrary: () => isFabricLoaderLibrary,
  isForgeLibrary: () => isForgeLibrary,
  isMissingVersionJsonError: () => isMissingVersionJsonError,
  isOptifineLibrary: () => isOptifineLibrary,
  isQuiltLibrary: () => isQuiltLibrary,
  isSameForgeVersion: () => isSameForgeVersion,
  launch: () => launch,
  launchServer: () => launchServer,
  matchVersion: () => matchVersion,
  parseForgeVersion: () => parseForgeVersion,
  parseOptifineVersion: () => parseOptifineVersion
});
...
var MinecraftFolder = class _MinecraftFolder {
  ...
  get mods() {
    return (0, import_path.join)(this.root, "mods");
  }
  get resourcepacks() {
    return (0, import_path.join)(this.root, "resourcepacks");
  }
  get assets() {
    return (0, import_path.join)(this.root, "assets");
  }
  get libraries() {
    return (0, import_path.join)(this.root, "libraries");
  }
  get versions() {
  ...
  getNativesRoot(version) {
    return (0, import_path.join)(this.getVersionRoot(version), version + "-natives");
  }
  getVersionRoot(version) {
    return (0, import_path.join)(this.versions, version);
  }
  getVersionJson(version) {
    return (0, import_path.join)(this.getVersionRoot(version), version + ".json");
  }
  getVersionServerJson(version) {
    return (0, import_path.join)(this.getVersionRoot(version), "server.json");
  }
  getVersionJar(version, type) {
    if (type === "client" || !type) return (0, import_path.join)(this.getVersionRoot(version), version + ".jar");
    if (type === "server")
      return this.getPath(
        "libraries",
        "net",
    ...
    return (0, import_path.join)(this.getVersionRoot(version), version + `-${type}.jar`);
  }
  getVersionAll(version) {
    return [
      (0, import_path.join)(this.versions, version),
      (0, import_path.join)(this.versions, version, version + ".json"),
      (0, import_path.join)(this.versions, version, version + ".jar")
    ];
  }
  getResourcePack(fileName) {
    return (0, import_path.join)(this.resourcepacks, fileName);
  }
  getMod(fileName) {
    return (0, import_path.join)(this.mods, fileName);
  }
  getLog(fileName) {
    return (0, import_path.join)(this.logs, fileName);
  }
  getMapInfo(map) {
  ...
  getLibraryByPath(libraryPath) {
    return (0, import_path.join)(this.libraries, libraryPath);
  }
  ...
  getPath(...path) {
    return (0, import_path.join)(this.root, ...path);
  }
};
var MinecraftPath;
((MinecraftPath2) => {
...
async function validateSha1(target, hash, strict = false) {
  if (await (0, import_promises.access)(target).then(
    () => false,
    () => true
  )) {
    return false;
...
async function checksum(target, algorithm) {
  const hash = (0, import_crypto.createHash)(algorithm).setEncoding("hex");
  try {
    await (0, import_promises2.pipeline)((0, import_fs.createReadStream)(target), hash);
  } catch (e) {
    if (e.code === "ENOENT") {
      return void 0;
    }
...
((LibraryInfo2) => {
  function resolveFromPath(path) {
    ...
    const ext = (0, import_path2.extname)(file);
...
((Version2) => {
  ...
  async function resolveDependency(path, version, platform2 = getPlatform()) {
    ...
    async function walk(versionName) {
      ...
      let contentString;
      try {
        contentString = await (0, import_promises3.readFile)(jsonPath, "utf-8");
      } catch (err) {
        const e = err;
...
((LaunchPrecheck2) => {
  ...
  async function linkAssets(resource, version, option) {
    ...
    const buf = await (0, import_promises4.readFile)(assetsIndexPath);
    const assetsIndex = JSON.parse(
      buf.toString()
    );
    const virtualPath = resource.getPath("assets/virtual/" + version.assets);
    await (0, import_promises4.mkdir)(virtualPath, { recursive: true }).catch(() => {
    });
    const dirs = Object.keys(assetsIndex.objects).map((path) => (0, import_path3.dirname)((0, import_path3.join)(virtualPath, path))).reduce((a, b) => a.add(b), /* @__PURE__ */ new Set());
    await Promise.all([...dirs].map((dir) => (0, import_promises4.mkdir)(dir, { recursive: true })));
    for (const [path, { hash }] of Object.entries(assetsIndex.objects)) {
      const assetPath = resource.getAsset(hash);
      const targetPath = (0, import_path3.join)(virtualPath, path);
      await (0, import_promises4.link)(assetPath, targetPath).catch((e) => {
        if (e.code !== "EEXIST") {
          throw e;
        }
      });
  ...
  async function checkNatives(resource, version, option) {
    ...
    await (0, import_promises4.mkdir)(native, { recursive: true }).catch((e) => {
      if (e.code !== "EEXIST") {
        throw e;
      }
    });
    ...
    const checksumFile = (0, import_path3.join)(native, ".json");
    const includedLibs = natives.map((n) => n.name).sort();
    const checksumFileObject = await (0, import_promises4.readFile)(checksumFile, "utf-8").then(JSON.parse).catch((e) => void 0);
    let shaEntries;
    ...
    async function extractJar(n) {
      ...
      const promises = [];
      const zip = await (0, import_unzip.open)(from, { lazyEntries: true, autoClose: false });
      for await (const entry of (0, import_unzip.walkEntriesGenerator)(zip)) {
        if (containsExcludes(entry.fileName) && notInMetaInf(entry.fileName) && notSha1AndNotGit(entry.fileName) && !entry.fileName.endsWith("/") && isSatisfyPlaform(entry.fileName)) {
          const fileName = (0, import_path3.basename)(entry.fileName);
          const dest = (0, import_path3.join)(native, fileName);
          if (fileName.indexOf("/") !== -1) {
            await (0, import_promises4.mkdir)((0, import_path3.dirname)(dest), {
              recursive: true
            }).catch((e) => {
            });
          }
          ...
          promises.push(
            (0, import_util.promisify)(import_stream.pipeline)(await (0, import_unzip.openEntryReadStream)(zip, entry), (0, import_fs2.createWriteStream)(dest))
          );
        }
      }
    ...
    if (shaEntries) {
      ...
      for (const entry of shaEntries) {
        ...
        const file = (0, import_path3.join)(native, entry.file);
      ...
      const entries = await Promise.all(
        extractedNatives.map(async (n) => ({
          ...n,
          sha1: await checksum((0, import_path3.join)(native, n.file), "sha1")
        }))
      );
      ...
      await (0, import_promises4.writeFile)(checksumFile, fileContent);
      const errors = result.map((r) => r.status === "rejected" ? r.reason : void 0).filter(isNotNull);
      if (errors.length === 0) {
        return;
      }
...
async function launchServer(options) {
  ...
  return (options.spawn ?? import_child_process.spawn)(args[0], args.slice(1), spawnOption);
}
function createMinecraftProcessWatcher(process2, emitter = new import_events.EventEmitter()) {
  var _a;
  let crashReport = "";
  ...
  (_a = process2.stdout) == null ? void 0 : _a.on("data", (s) => {
    ...
    if (string.indexOf("---- Minecraft Crash Report ----") !== -1) {
      ...
      crashReportLocation = crashReportLocation.replace(import_os.EOL, "").trim();
    } else if (string.indexOf("Crash report saved to ") !== -1) {
      crashReportLocation = string.substring(
        string.indexOf("Crash report saved to ") + "Crash report saved to ".length
      );
...
async function launch(options) {
  var _a;
  const gamePath = !(0, import_path3.isAbsolute)(options.gamePath) ? (0, import_path3.resolve)(options.gamePath) : options.gamePath;
  ...
  if (!(0, import_fs2.existsSync)(gamePath)) {
    await (0, import_promises4.mkdir)(gamePath);
  }
  return (options.spawn ?? import_child_process.spawn)(args[0], args.slice(1), spawnOption);
}
...
function generateArgumentsServer(options, _delimiter = import_path3.delimiter, _sep = import_path3.sep) {
  const {
    javaPath,
    minMemory,
    maxMemory,
  ...
  if (options.classPath && options.classPath.length > 0) {
    cmd.push("-cp", options.classPath.map((v) => v.replaceAll(import_path3.sep, _sep)).join(_delimiter));
  }
  if (options.serverExectuableJarPath) {
    cmd.push("-jar", options.serverExectuableJarPath.replaceAll(import_path3.sep, _sep));
...
async function generateArguments(options) {
  ...
  const cmd = [];
  const { id = (0, import_crypto2.randomUUID)().replace(/-/g, ""), name = "Steve" } = options.gameProfile || {};
  const accessToken = options.accessToken || (0, import_crypto2.randomUUID)().replace(/-/g, "");
  ...
  if (!gameIcon) {
    ...
    const indexContent = await (0, import_promises4.readFile)(index, { encoding: "utf-8" }).then(
  ...
  const jvmOptions = {
    ...
    ].map((c) => c.replaceAll("\\", "/")).join(import_path3.delimiter),
    library_directory: mc.getPath("libraries").replaceAll("\\", "/"),
    classpath_separator: import_path3.delimiter,
  ...
  const assetsDir = (0, import_path3.join)(resourcePath, "assets");
  ...
  const mcOptions = {
    ...
    game_assets: (0, import_path3.join)(assetsDir, "virtual", version.assets).replaceAll("\\", "/"),
